...
# Export additional relevant registry branches
$additionalRegBranches = @(
    "HKCU\Software",                    # User-specific application settings
    "HKCU\Control Panel",               # User-specific Windows settings
    "HKCU\Software\Classes",            # Shell extensions and context menu items
    "HKCU\Software\Microsoft\Windows",  # User-specific Windows settings
    "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies"  # User-specific security settings
)

foreach ($branch in $additionalRegBranches) {
    $destinationAdditionalRegFile = "$exportPath\$userSID-$($branch.Replace('\', '-')).reg"
    reg export $branch $destinationAdditionalRegFile
}

# Output SID and file information
Write-Host "Registry branches exported to: $exportPath"
...
robocopy "$exportPath" "$destinationRegExport" /E /NFL /NJH /NJS /R:0 /W:0 /MOVE /XJ /TEE
